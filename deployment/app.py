...
def main():
    # Main header
    st.title("🌴 Tourism Package Predictor ✈️")

    # Subtitle
    st.markdown("### Predict whether a customer will purchase the **Wellness Tourism Package** based on their profile and preferences.")
    st.markdown("---")

    # Load model
    model = load_model()
    if model is None:
        st.error("Model could not be loaded. Please check the model availability.")
        return
    ...
    st.caption("Developed for Visit with Us travel company")
